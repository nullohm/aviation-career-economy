...
{
    public partial class YearlyEarningsReportWindow : Window
    {
        private readonly IDailyEarningsRepository _dailyEarningsRepository;

        public YearlyEarningsReportWindow(int year, IDailyEarningsRepository dailyEarningsRepository)
        {
        ...
        private void LoadDetails(int year)
        {
            TxtDate.Text = year.ToString();

            var details = _dailyEarningsRepository.GetDetailsByYear(year);

            var maxMonthlyProfit = details
                .GroupBy(d => d.Date.Month)
                .Select(g => Math.Abs(g.Sum(d => d.Profit)))
                .DefaultIfEmpty(1m)
                .Max();

            var monthlySummaries = details
                .GroupBy(d => d.Date.Month)
                .Select(g => new MonthlyYearlySummary
                {
                    Month = g.Key,
                    ...
                    MaxProfit = maxMonthlyProfit
                })
                .OrderBy(m => m.Month)
                .ToList();

            DetailsGrid.ItemsSource = monthlySummaries;
            ...
            TxtTotalHours.Text = totalHours.ToString("N0");
    ...
    public class MonthlyYearlySummary
    {
        ...
        public double PerformanceWidth => MaxProfit > 0 ? (double)(Math.Abs(Profit) / MaxProfit) * 150 : 0;
    }
}
